...
public class FollowRail : MonoBehaviour
{
    ...
    void Update() {
        ...
        transform.localScale = new Vector3(0.4f / rail.localScale.x, 0.2f / rail.localScale.y, 0.2f / rail.localScale.z);
    ...
    void OnTriggerEnter(Collider other) {
        ...
        if (inRail.tag == "Rail") {
            ...
            /*Rigidbody railRB = transform.GetComponent<Rigidbody>();
            if (inRail.name != "Drop Rail") {
                if (railRB.constraints == RigidbodyConstraints.FreezePositionX){
                    railRB.constraints = RigidbodyConstraints.None;
                    railRB.constraints = RigidbodyConstraints.FreezePositionY;
                    railRB.constraints = RigidbodyConstraints.FreezeRotationX;
                    railRB.constraints = RigidbodyConstraints.FreezeRotationX; 
                }
                transform.SetParent(rail); 
            } else {
                railRB.constraints = RigidbodyConstraints.FreezeAll;
            }
            ...
            //SSStransform.position = new Vector3(transform.position.x, inRail.position.y, inRail.position.z);
            ...
            //transform.rotation = new Quaternion(transform.rotation.x, inRail.rotation.y, inRail.rotation.z,SS  1);
